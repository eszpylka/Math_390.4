...
Source the simple dataset from lecture 6p:
...
1. Try your best to write a general perceptron learning algorithm to the following `Roxygen` spec. For inspiration, see the one I wrote in lecture 6.
...
2. Use the `e1071` package to fit an SVM model to `y_binary` using the predictors found in `X_simple_feature_matrix`. Do not specify
...
4. We move on to simple linear modeling using the ordinary least squares algorithm.
...
Solve for the least squares line by computing $b_0$ and $b_1$ _without_ using the functions `cor`, `cov`, `var`, `sd` but instead computing it from the $x$ and $y$ quantities manually. See the notes for the derivation.
...
```{r}
lm_mod = #TO-DO
expect_equal(b_0, coef(lm_mod)[1])
expect_equal(b_1, coef(lm_mod)[2])
```

5. We are now going to repeat one of the first linear model building exercises in history - that of Sir Francis Galton in 1886. First load up package `HistData`.
...
Note that in Math 241 you learned that the sample average is an estimate of the "mean", the population expected value of height. We will call the average the "mean" going forward.

Run a linear model attempting to explain the childrens' height using the parents' height. Use `lm` and use the R formula notation. Report $b_0$, $b_1$, RMSE and $R^2$.
...
How good is this model? How well does it predict?

TO-DO

Now use the code from practice lecture 8 to plot the data and a best fit line using package `ggplot2`.

```{r}
#TO-DO
```

It is reasonable to assume that parents and their children have the same height. Explain why.
...
Let's plot (a) the data, (b) your least squares line defined by $b_0$ and $b_1$ in blue, (c) the theoretical line $\beta_0$ and $\beta_1$ if the parent-child height equality held in red and (d) the mean height in green.
...
Why is modeling with $y$ continuous called "regression"?
